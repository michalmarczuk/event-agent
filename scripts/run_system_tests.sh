...
scenarios='happy_path telegram_failure no_events previously_seen_event canceled_event_filtering openai_failure multiple_events ticketmaster_failure invalid_recommendation_id mixed_source_discovery'
...
if [ "$qase_result_count" -ne 10 ] || [ ! -f "$qase_report_dir/run.json" ]; then
    echo "Expected local Qase results for exactly ten System Tests." >&2
    exit 1
fi
